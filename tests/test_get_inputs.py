...
test_get_grid_around_parameters = [
    ...
    ((1, 1), 1, np.asarray([[E, E, E], [E, C, E], [E, E, E]])),
    ((2, 3), 1, np.asarray([[E, E, E], [E, C, E], [E, E, E]])),
    ((2, 1), 1, np.asarray([[E, E, E], [E, C, E], [E, E, E]])),
    ((1, 3), 1, np.asarray([[E, E, E], [E, C, E], [E, E, E]])),
    # Check actual corners to see the grid applies padding correctly
    ((0, 0), 1, np.asarray([[W, W, W], [W, C, E], [W, E, E]])),
    ((3, 4), 1, np.asarray([[E, E, W], [E, C, W], [W, W, W]])),
    ((0, 4), 1, np.asarray([[W, W, W], [E, C, W], [E, E, W]])),
    ((3, 0), 1, np.asarray([[W, E, E], [W, C, E], [W, W, W]])),
    # Check the middles of each grid edge
    ((0, 2), 1, np.asarray([[W, W, W], [E, C, E], [E, E, E]])),
    ((3, 2), 1, np.asarray([[E, E, E], [E, C, E], [W, W, W]])),
    ((1, 4), 1, np.asarray([[E, E, W], [E, C, W], [E, E, W]])),
    ((1, 0), 1, np.asarray([[W, E, E], [W, C, E], [W, E, E]])),
...
def test_get_grid_around(
    ...
    with grid:
        grid_around = extract_inputs.get_instance_grid(
            grid=grid, pos=Position(*pos), radius=radius, layers=layers
        )
    ...
    assert (
        grid_around[:, :, layers["Critter"]].sum() == 1
    ), "There should only be one critter on the map!"
    assert (
        grid_around[:, :, layers["Food"]].sum() == 0
...
def test_instance_grid_is_cached() -> None:
    ...
    with grid as locked_grid:
        extract_inputs.get_instance_grid(
            locked_grid, pos=Position(1, 1), radius=5, layers=Critter.LAYERS
        )
    ...
    with grid as locked_grid:
        extract_inputs.get_instance_grid(
            locked_grid, pos=Position(1, 1), radius=10, layers=Critter.LAYERS
        )
    ...
    with grid as locked_grid:
        extract_inputs.get_instance_grid(
            locked_grid, pos=Position(1, 1), radius=10, layers={"Critter": 1}
        )
    validate_cache_changes(True, last_cache)
    ...
    with grid as locked_grid:
        extract_inputs.get_instance_grid(
            locked_grid, pos=Position(1, 2), radius=10, layers={"Critter": 1}
        )
    validate_cache_changes(False, last_cache)
